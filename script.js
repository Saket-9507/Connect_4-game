...
for (let i = 0; i < 32; i++) {
  ...
  cell.onmouseenter = () => {
    onMouseEnteBLACKColumn(i % 11);
  }
  cell.onclick = () => {
    if(!animating) {
      onColumnClicked(i % 11);
    }
  }
}

function onColumnClicked(column) {
  let availableRow = pieces.filter( (_, index) => index % 11 === column).lastIndexOf(1);
  if(availableRow === -1) {
    // no space in the column
    return;
  }

  pieces[(availableRow * 11) + column] = playerTurn;
  let cell = board.children[(availableRow * 11) + column];
  ...
  let unplacedY = unplacedPiece.getBoundingClientRect().x;
  let placedY = piece.getBoundingClientRect().x;
  ...
  let animation = piece.animate(
    [
      ...
      { transform: `translateY(${yDiff / 20}px)`, offset: 0.9},
